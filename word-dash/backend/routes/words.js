...
function getRandomFallbacks(usedWords, count, wordLength = 5) {
  const used = new Set((usedWords || []).map(w => w.toUpperCase()));
  const byLength = FALLBACK_WORDS.filter(w => w.word.length === wordLength && !used.has(w.word));
  const pool = byLength.length >= count ? byLength : FALLBACK_WORDS.filter(w => !used.has(w.word));
  const final = pool.length >= count ? pool : [...FALLBACK_WORDS];
  return final.sort(() => Math.random() - 0.5).slice(0, count);
}

router.post('/', async (req, res) => {
  const { lang = 'en', langName = 'English', count = 6, usedWords = [], wordLength = 5 } = req.body;
  const apiKey = process.env.ANTHROPIC_API_KEY;

  if (!apiKey) {
    console.log('[words] No ANTHROPIC_API_KEY — using fallback words');
    return res.json({ words: getRandomFallbacks(usedWords, count, wordLength), fallback: true });
  }
  console.log('[words] Fetching AI words, lang=' + lang + ' count=' + count + ' length=' + wordLength);
  ...
  const buildPrompt = (needed, alreadyUsed) => {
    ...
    return 'Generate exactly ' + needed + ' words for a Wordle-style game. Language: ' + langName + '.\n\n'
      ...
      + '1. EXACTLY ' + wordLength + ' letters. Count every letter carefully before submitting.\n'
      ...
      + '6. Use varied categories — avoid overusing common words.\n'
      + '7. Hints and categories must be in English.\n\n'
      + 'Example: a ' + wordLength + '-letter word must have EXACTLY ' + wordLength + ' characters.\n\n'
  ...
  const callAI = async (prompt) => {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model:      isEn ? 'claude-haiku-4-5-20251001' : 'claude-sonnet-4-6',
        max_tokens: 800,
  ...
  const validateWords = (raw) =>
    raw
      ...
      .filter(w => w.word.length === wordLength && w.category && w.hint);
  ...
  try {
    ...
    if (valid.length < count) {
      const fallbackPad = getRandomFallbacks([...allUsed, ...valid.map(w => w.word)], count - valid.length, wordLength);
    ...
    res.json({ words: getRandomFallbacks(usedWords, count, wordLength), fallback: true });
  }
});
