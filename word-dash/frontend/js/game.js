...
function fetchWordsFromAI(count=6){
  ...
  .then(data=>{
    ...
    if(!valid.length) throw new Error('no valid words after filter');
    console.log('[words] Got '+valid.length+' AI words, fallback='+!!data.fallback);
    wordQueue.push(...valid);
  })
  .catch(e=>{
    console.warn('[words] fetch failed:',e.message,'— using local fallback');
    // Shuffle fallback so order is always random
    const unused=FALLBACK_WORDS.filter(w=>!usedWords.has(w.word));
    const pool=(unused.length>=count?unused:FALLBACK_WORDS).slice().sort(()=>Math.random()-.5);
