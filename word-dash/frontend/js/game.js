...
let isMultiplayer = false, isRanked = false, usedWords = new Set();

// ── Rank System ──
const RANK_TIERS = [
  { name:'Bronze',   min:800,  max:1199, color:'#cd7f32', icon:'🥉' },
  { name:'Silver',   min:1200, max:1399, color:'#c0c0c0', icon:'🥈' },
  { name:'Gold',     min:1400, max:1599, color:'#ffd700', icon:'🥇' },
  { name:'Platinum', min:1600, max:1799, color:'#00e5ff', icon:'💎' },
  { name:'Diamond',  min:1800, max:9999, color:'#b9f2ff', icon:'💠' },
];

function getRankTier(rp) {
  const t = RANK_TIERS.find(t => rp >= t.min && rp <= t.max) || RANK_TIERS[0];
  const range = t.max - t.min + 1;
  const third = Math.floor(range / 3);
  const pos = rp - t.min;
  const div = pos < third ? 'III' : pos < third * 2 ? 'II' : 'I';
  return { ...t, division: div, label: t.name + ' ' + div };
}

function calcRPChange(won, guesses, isMpRanked) {
  if (isMpRanked) return won ? 30 : -20;
  if (!won) return -10;
  return Math.max(15, 25 - (guesses - 1) * 2);
}

function getRankProgressPct(rp) {
  const t = getRankTier(rp);
  const range = t.max - t.min + 1;
  return Math.min(100, Math.floor(((rp - t.min) / range) * 100));
}
...
const TIME_LIMIT = 60, ROWS = 6;
const MIN_WORD_LEN = 2, MAX_WORD_LEN = 10;
let COLS = 5; // dynamic per round

function pickWordLength() {
  return Math.floor(Math.random() * (MAX_WORD_LEN - MIN_WORD_LEN + 1)) + MIN_WORD_LEN;
}
...
function showLoadingState(on){
  ...
  if(badge) badge.textContent = on ? 'LOADING WORD...' : ((category||'') + ' · ' + COLS + ' letters');
...
async function pickWord(){
  ...
  if(wordQueue.length===0){
    ...
    await fetchWordsFromAI(6, wordLength);
    showLoadingState(false);
  } else if(wordQueue.length<=2){
    fetchWordsFromAI(6, wordLength); // pre-fetch in background
  }

  // Try to find a word in the queue matching the desired length
  let pickIdx = wordQueue.findIndex(w => w.word.length === wordLength);
  if(pickIdx === -1) {
    // No matching length in queue — fetch specifically for this length
    showLoadingState(true);
    await fetchWordsFromAI(4, wordLength);
    showLoadingState(false);
    pickIdx = wordQueue.findIndex(w => w.word.length === wordLength);
  }

  if(pickIdx === -1){
    // Last resort: fallback word closest to desired length
    const pool = FALLBACK_WORDS.slice().sort((a,b) =>
      Math.abs(a.word.length - wordLength) - Math.abs(b.word.length - wordLength)
    );
    const fb = pool[0];
  ...
  const pick = wordQueue.splice(pickIdx, 1)[0];
  usedWords.add(pick.word);
  COLS = pick.word.length;
  saveUsedWords(currentLang);
  return pick;
}

function fetchWordsFromAI(count=6, wordLength=5){
  if(fetchPromise)return fetchPromise;
  const lang=LANGUAGES.find(l=>l.code===currentLang)||LANGUAGES[0];
  fetchPromise=fetch('/api/words',{
    method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({lang:currentLang,langName:lang.name,count:count,usedWords:[...usedWords],wordLength:wordLength})
  })
  ...
  .then(data=>{
    ...
    const valid=data.words
      ...
      .filter(w=>w.word.length===wordLength&&/^[A-ZÀ-ɏÆØÅ]+$/i.test(w.word));
    const toAdd=valid.length?valid:data.words.map(w=>({...w,word:w.word.toUpperCase().trim()})).filter(w=>w.word.length===wordLength);
...
function launchGame(mp){
  // Always derive COLS from the actual word — never trust the pre-picked random length
  COLS = target.length;
...
function buildGrid(){
  ...
  // Scale font size: big for short words, small for long words
  const fontSize = COLS <= 3 ? '2.4rem' : COLS <= 5 ? '1.8rem' : COLS <= 7 ? '1.4rem' : '1.1rem';
...
function endGame(won,reason=''){
  ...
  const rpChange = isRanked ? calcRPChange(won, currentRow+1, isMultiplayer) : 0;
  if(currentUser){
    ...
    if(isRanked && rpChange !== 0){
      currentUser.rankPoints = Math.max(800, (currentUser.rankPoints||1000) + rpChange);
      const tier = getRankTier(currentUser.rankPoints);
      setTimeout(()=>showUnlockToast((rpChange>0?'📈 +':'📉 ')+rpChange+' RP — '+tier.label),2200);
    }
    if(!currentUser.isGuest){
      API.saveProgress({
        won:won, guesses:currentRow, timeLeft:timeLeft,
        xpEarned:xpEarned, lang:currentLang, isMp:isMultiplayer,
        isRanked:isRanked, isMpRanked:isMultiplayer&&isRanked,
  ...
  setTimeout(()=>{
    ...
    // Show RP change if ranked
    const rpEl = document.getElementById('rp-gained');
    if(rpEl){
      if(isRanked && rpChange!==0){
        rpEl.textContent=(rpChange>0?'+':'')+rpChange+' RP';
        rpEl.style.color=rpChange>0?'var(--correct)':'#f04040';
...
async function startSolo(ranked = false) {
  isMultiplayer = false;
  isRanked = ranked;
  const btn = ranked
    ? document.querySelector('[onclick="startRanked()"]')
    : document.querySelector('[onclick="startSolo()"]');
  ...
  if(btn && ranked){ btn.innerHTML='<div class="icon">🏆</div><div class="label">Ranked</div><div class="sub">gain/lose RP</div>'; btn.disabled=false; }
  if(btn && !ranked){ btn.innerHTML='<div class="icon">🎮</div><div class="label">Solo Play</div><div class="sub">vs the clock</div>'; btn.disabled=false; }
  launchGame(false);
}

function startRanked() {
  if(!currentUser || currentUser.isGuest) {
    showMessage('SIGN IN TO PLAY RANKED', 2000);
    return;
  }
  startSolo(true);
}
