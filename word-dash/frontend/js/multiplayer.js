...
function initPeer(callback){
  closePeer();
  // Use our own self-hosted PeerJS server — reliable, no external dependency
  ...
  const timeout = setTimeout(()=>{
    showMessage('Connection timeout. Try again.');
    closePeer();
  }, 15000);
  ...
  peer.on('error', e=>{
    ...
    const msgs = {
      ...
      'socket-error':     'Could not reach server. Try again.',
    };
    showMessage(msgs[e.type] || 'Connection error: ' + e.type);
    console.warn('PeerJS error:', e.type, e);
  });
